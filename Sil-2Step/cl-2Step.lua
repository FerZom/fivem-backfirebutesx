	...
	if antilag == false then
		TriggerEvent("mythic_progbar:client:progress", {
            ...
            if not cancelled then
                antilag = true
				exports['mythic_notify']:SendAlert('info', 'Egzoz Açıldı', 3000)
            end
        end)
	else
		TriggerEvent("mythic_progbar:client:progress", {
            ...
            if not cancelled then
                antilag = false
				exports['mythic_notify']:SendAlert('info', 'Egzoz Kapandı', 3000)
            end
		end)
	end
end)
